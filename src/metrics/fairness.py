...
class StatisticScores(BaseMetric):
    ...
    def get(self, normalize=False):
        support = self.stat_scores.tp + self.stat_scores.fp + \
            self.stat_scores.tn + self.stat_scores.fn
...
class GroupFairnessMetric(BaseMetric):
    def __init__(self, **kwargs):
        ...
        assert len(group_ids[self.group_name]
                   ) > 1, "group_ids must have at least 2 groups"
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ...
    def calculate(self, y_pred, y_true, group_ids: dict):
        ...
        y_pred = y_pred.to(self.device)
        y_true = y_true.to(self.device)
        assert len(y_pred) == len(y_true) == len(
            current_group_ids), "y_pred, y_true and group_ids must have the same length"
        ...
        if not self.use_multiclass:
            ...
            for group_id in torch.unique(current_group_ids):
                if group_id != -1:
                    ...
                    self.stats_per_group[group_id.item()].calculate(
            ...
            for current_class in range(self.num_classes):
                ...
                for group_id in torch.unique(current_group_ids):
                    if group_id != -1:
                        ...
                        self.stats_per_class[current_class][group_id.item()].calculate(
                            y_pred_group, y_true_group)
    ...
    def get(self):
        if not self.use_multiclass:
            ...
            for current_class in range(self.num_classes):
                ...
                dp = self._reduction(torch.tensor(
                    self.stats_per_class_group_diff[current_class]))
...
class EqualOpportunity(GroupFairnessMetric):
    ...
    def get(self):
        if not self.use_multiclass:
            ...
            for i in range(len(group_ids)):
                for j in range(i+1, len(group_ids)):
                    self.stats_per_group_diff.append(abs(self.stats_per_group[group_ids[i]].get()[
                                                     'tpr'] - self.stats_per_group[group_ids[j]].get()['tpr']))
            ...
            for current_class in range(self.num_classes):
                ...
                eo = self._reduction(torch.tensor(
                    self.stats_per_class_group_diff[current_class]))
...
class EqualizedOdds(GroupFairnessMetric):
    ...
    def get(self):
        if not self.use_multiclass:
            ...
            for i in range(len(group_ids)):
                for j in range(i+1, len(group_ids)):
                    self.stats_per_group_diff_tpr.append(abs(self.stats_per_group[group_ids[i]].get()[
                                                         'tpr']-self.stats_per_group[group_ids[j]].get()['tpr']))
                    self.stats_per_group_diff_fpr.append(abs(self.stats_per_group[group_ids[i]].get()[
                                                         'fpr']-self.stats_per_group[group_ids[j]].get()['fpr']))
