...
class BaseBinarySurrogate:
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.weight = kwargs.get('surrogate_weight')
        print('surrogate self.weight:', self.weight)
    ...
    def _calculate(self, logits, positive_mask, negative_mask):
        ...
        positive_surrogate = torch.mean(
            torch.tanh(torch.relu(positive_probabilities)))
        negative_surrogate = torch.mean(
            torch.tanh(torch.relu(negative_probabilities)))
...
class BaseSurrogate:
    def __init__(self, **kwargs: Any) -> None:
        ...
        assert isinstance(self.unique_group_ids,
                          dict), f'unique_group_ids should be a dictionary'
        self.group_name: str = kwargs.get('group_name')
        assert self.group_name in self.unique_group_ids.keys(
        ), f'{self.group_name} is not a valid group_name'
        self.reduction = kwargs.get('reduction', 'mean')
        assert self.reduction in [
            'min', 'max', 'mean'], f'{self.reduction} is not a valid reduction'
        ...
        for i in range(len(current_group_ids)):
            for j in range(i+1, len(current_group_ids)):
                self.surrogates.append(surrogate_class(
                    ...
                    surrogate_weight=self.weight,
                    surrogate_name=self.name
                ))
