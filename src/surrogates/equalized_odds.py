...
class BinaryEqualizedOddsSurrogate(BaseBinarySurrogate):
    ...
    def _compute_statistic(self, logits, labels, group_ids):
        positive_mask = ((group_ids[self.group_name] == self.positive_group_id) & (
            labels == 1)).squeeze()
        negative_mask = ((group_ids[self.group_name] == self.negative_group_id) & (
            labels == 1)).squeeze()
        surrogate_opportunity = self._calculate(
            logits, positive_mask, negative_mask)
        positive_mask = ((group_ids[self.group_name] == self.positive_group_id) & (
            labels == 0)).squeeze()
        negative_mask = ((group_ids[self.group_name] == self.negative_group_id) & (
            labels == 0)).squeeze()
        surrogate_equality = self._calculate(
            logits, positive_mask, negative_mask)

        surrogate_odds = torch.max(torch.cat(
            (surrogate_opportunity.view(1, -1), surrogate_equality.view(1, -1)), dim=1))
