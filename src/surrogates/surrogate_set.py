...
class SurrogateFunctionSet:
    def __init__(self, surrogates: list[BaseSurrogate], **kwargs) -> None:
        ...
        self.surrogate_dict: dict = {
            surrogate.name: surrogate for surrogate in surrogates}
        self.total_weight: float = sum(
            [surrogate.weight for surrogate in surrogates])
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.p = kwargs.get('p', 2)
        self.weights = torch.stack([torch.tensor(
            surrogate.weight, dtype=torch.float) for surrogate in self.surrogates])
