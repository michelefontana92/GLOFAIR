...
class DataModule(BaseDataLoader):
    ...
    def val_loader(self, batch_size=None):
        return DataLoader(self.datasets.get('val'),
                          batch_size=len(self.datasets.get(
                              'val')) if batch_size is None else batch_size,
                          num_workers=self.num_workers,
                          shuffle=False)
    ...
    def test_loader(self, batch_size=None):
        return DataLoader(self.datasets.get('test'),
                          batch_size=len(self.datasets.get(
                              'test')) if batch_size is None else batch_size,
                          num_workers=self.num_workers,
                          shuffle=False)
    ...
    def train_loader_eval(self, batch_size=None):
        return DataLoader(self.datasets.get('train'),
                          batch_size=len(self.datasets.get(
                              'train')) if batch_size is None else batch_size,
                          num_workers=self.num_workers,
                          shuffle=False)
    ...
    def deserialize(data):
        ...
        instance.datasets = {
            key: DatasetFactory.deserialize(
                dataset_data) if dataset_data else None
