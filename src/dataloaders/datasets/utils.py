...
def assign_group_id(df, sensitive_attributes):
    ...
    for (name, group_dict) in sensitive_attributes:
        ...
        combination_ids = {tuple(combination): idx for idx,
                           combination in enumerate(combinations)}

        # Creiamo il dizionario id: combinazione
        id_to_combination[name] = {idx: dict(
            zip(columns, combination)) for combination, idx in combination_ids.items()}
...
def assign_group_id_old(data, sensitive_dict, group_name):
    ...
    for key in sensitive_dict.keys():
        if type(data[key].iloc[0]) == str:
            ...
            # lambda x: x if x in sensitive_dict[key] else 'NoneGroup')
            data[key +
                 '_new'] = data[key].apply(lambda x: _assign(x, sensitive_dict, key))
            ...
            data[key +
                 '_new'] = data[key].apply(lambda x: find_bucket(x, thresholds))
