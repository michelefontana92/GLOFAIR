...
class BaseDataset(Dataset):
        ...
        if self.use_local_weights:
            for group_name, _ in self.sensitive_attributes:
                self.local_weights[group_name] = torch.from_numpy(
                    local_weights[group_name]).float()
        self.data = None
    ...
    def _preprocess(self, data):
        ...
        for idx, label in enumerate(self.labels):
            data_cpy[self.target] = data_cpy[self.target].replace(
                label, idx).infer_objects()

        data_cpy = self.data_preprocessing(data_cpy)
        print('Assigning group ids')
        # for group_name,sensitive_dict in self.sensitive_attributes:
        data_cpy, id_to_combination_dict = assign_group_id(
            data_cpy, self.sensitive_attributes)
    ...
    def _load_dataset(self, load_data=True):
        ...
        self.class_weights = self._compute_class_weight(
            data) if self.use_class_weights else None
        ...
        drop_cols += [f'group_id_{group_name}' for group_name,
                      _ in self.sensitive_attributes]
        if self.use_local_weights:
            drop_cols += [f'local_weights_{group_name}' for group_name,
                          _ in self.sensitive_attributes]
        ...
        for group_name, _ in self.sensitive_attributes:
            ...
            group_ids[group_name] = torch.tensor(
                [i for i in list(data[f'group_id_{group_name}'].unique())]).view(1, -1)
            if self.use_local_weights:
    ...
    def __getitem__(self, index):
        groups = {group_name: self.groups[group_name][index]
                  for group_name, _ in self.sensitive_attributes}

        if self.use_local_weights:
            local_weights = {
                group_name: self.local_weights[group_name][index] for group_name, _ in self.sensitive_attributes}

        groups_tensor = {
            group_name: self.groups_tensor[group_name][index] for group_name, _ in self.sensitive_attributes}
    ...
    def _compute_local_reweighing(self, df: pd.DataFrame, group_name: str, sensitive_dict: dict):
        ...
        data, id_to_combination_dict = assign_group_id(
            data, self.sensitive_attributes)
        self.id_to_combination_dict = id_to_combination_dict
        ...
        for group_id in weights.keys():
            ...
            for attribute_name, _ in attributes.items():
                ...
                joint_proba *= len(data[data[attribute_name]
                                   == value]) / len(data)
        ...
        data[f'local_weights_{group_name}'] = data[idx].apply(
            lambda x: weights[x])
    ...
    def merge(self, dataset):
        ...
        self.data = pd.concat([data_src, data_dest]).sample(
            frac=1).reset_index(drop=True)
        assert len(data_src.columns) == len(data_dest.columns) == len(
            self.data.columns), 'Columns mismatch'
        assert len(self.data) == len(data_src) + \
            len(data_dest), 'Data mismatch'
