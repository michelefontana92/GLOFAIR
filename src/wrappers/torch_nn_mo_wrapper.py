...
class TorchNNMOWrapper(TorchNNWrapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.surrogate_functions: list[(callable, float)] = kwargs.get(
            'surrogate_functions')
    ...
    def _training_step(self, batch, batch_idx):
        ...
        group_ids = {group_name: group_ids[group_name].to(
            self.device) for group_name in group_ids.keys()}
        group_ids_list = {group_name: group_ids_list[group_name].to(
            self.device) for group_name in group_ids_list.keys()}
    ...
    def _evaluate_requirements(self, data_loader):
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(data_loader):
                self.model.eval()
                loss, output, target, prediction = self._validation_step(
                    batch, batch_idx)
            ...
            groups_dict = {group_name: torch.cat([batch[group_name] for batch in groups], dim=0).detach(
            ).cpu() for group_name in groups[0].keys()}
    ...
    def _update_metrics(self, **kwargs):
        ...
        val_requirements, val_loss, val_outputs, val_targets, val_predictions, val_groups_dict = self._evaluate_requirements(
            val_loader)
        train_requirements, train_loss, train_outputs, train_targets, train_predictions, train_groups_dict = self._evaluate_requirements(
            train_loader_eval)
