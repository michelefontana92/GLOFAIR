...
class TorchNNWrapper(BaseWrapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert len(list(self.model.parameters())
                   ) > 0, "Model has no parameters to optimize"
        ...
        assert isinstance(
            self.optimizer, torch.optim.Optimizer), "Optimizer should be an instance of torch.optim.Optimizer"
        assert issubclass(
            self.model.__class__, torch.nn.Module), "Model should be an instance of torch.nn.Module"
        assert issubclass(
            self.logger.__class__, BaseLogger), "Logger should be an instance of BaseLogger"
        assert issubclass(self.data_module.__class__,
                          DataModule), "DataModule should be an instance of DataModule"
        assert isinstance(self.checkpoints,
                          list), "Checkpoints should be a list of callbacks"
        for checkpoint in self.checkpoints:
            assert issubclass(checkpoint.__class__, EarlyStopping) or issubclass(
                checkpoint.__class__, ModelCheckpoint), "Checkpoints should be instances of EarlyStopping or ModelCheckpoint"

        assert isinstance(self.metrics, list), "Metrics should be a list"
        for metric in self.metrics:
            assert issubclass(
                metric.__class__, BaseMetric), "Metrics should be instances of BaseMetric"
        assert isinstance(
            self.num_epochs, int), "num_epochs should be an integer"
        assert isinstance(self.verbose, bool), "verbose should be a boolean"
        assert isinstance(
            self.device, torch.device), "device should be an instance of torch.device"
        ...
        for metric in metrics:
            ...
            if issubclass(metric.__class__, GroupFairnessMetric):
                ...
                metric.calculate(y_pred.detach().cpu(),
                                 ...
                                 group_ids_detached)
                """
                            tmp_result.update(metric.compute_wasserstein_distance(logits=logits.detach().cpu(),
                                                                 labels=y_true.detach().cpu(),
                                                                 group_ids=group_ids_detached,
    ...
    def compute_stat_per_group(self, data_loader,
        ...
        for batch_idx, batch in enumerate(data_loader):
            _, _, targets, predictions = self._validation_step(
                batch, batch_idx)
        if issubclass(metric.__class__, GroupFairnessMetric):
        ...
        try:
            for epoch in tqdm.tqdm(range(n_rounds)):
                ...
                for batch_idx, batch in enumerate(val_loader):
                    loss, outputs, targets, predictions = self._validation_step(
                        batch, batch_idx)
                    val_loss += loss
                ...
                for batch_idx, batch in enumerate(train_loader_eval):
                    _, outputs, targets, predictions = self._validation_step(
                        batch, batch_idx)
                ...
                val_predictions = torch.cat(
                    val_predictions, dim=0).detach().cpu()
                val_groups_dict = {group_name: torch.cat([batch[group_name] for batch in val_groups], dim=0).detach(
                ).cpu() for group_name in val_groups[0].keys()}
                ...
                train_predictions = torch.cat(
                    train_predictions, dim=0).detach().cpu()
                train_groups_dict = {group_name: torch.cat([batch[group_name] for batch in train_groups], dim=0).detach(
                ).cpu() for group_name in train_groups[0].keys()}
    ...
    def set_params(self, model_src):
        assert isinstance(
            model_src, torch.nn.Module), "model_src should be an instance of torch.nn.Module"
    ...
    def set_params_from_dict(self, model_src_dict):
        assert isinstance(
            model_src_dict, dict), "model_src_dict should be a dictionary"
        model_dst_dict = self.model.state_dict()
        for key in model_src_dict.keys():
            model_dst_dict[key] = model_src_dict[key]
        self.model.load_state_dict(model_dst_dict)
    ...
    def model_checkpoint(self, models_list):
        ...
        for idx, model in enumerate(models_list):
            ...
            val_loader = self.data_module.val_loader()
            initial_metrics = self.score(
                val_loader, self.metrics, prefix='val')
