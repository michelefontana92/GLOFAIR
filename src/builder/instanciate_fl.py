...
class GLOFAIR_Builder:
    ...
    def __init__(self, **kwargs):
        ...
        assert isinstance(self._server_config,
                          dict), "server_config must be a dictionary"
        assert isinstance(self._clients_config,
                          list), "clients_config must be a list"
        assert isinstance(self._algorithm_config,
                          dict), "algorithm_config must be a dictionary"
        ...
        self.optimizer_name = self._algorithm_config.get(
            'optimizer_name', 'Adam')
        self.lr = self._algorithm_config.get('lr', 1e-3)
        self.training_group_name = self._algorithm_config.get(
            'training_group_name')
        self.num_local_epochs = self._algorithm_config.get(
            'num_local_epochs', 3)
        self.num_federated_epochs = self._algorithm_config.get(
            'num_federated_epochs', 2)
        self.fine_tune_epochs = self._algorithm_config.get(
            'fine_tune_epochs', 50)
        self.verbose = self._algorithm_config.get('verbose', False)
        self.surrogate_set: SurrogateFunctionSet = self._algorithm_config.get(
            'surrogate_set')
        self.requirement_set: RequirementSet = self._algorithm_config.get(
            'requirement_set')
        self.clients = []
        ...
        s_ckpt_dir = self._server_config.get(
            'checkpoint_dir', f'checkpoints/{self._project}')
        s_ckpt_name = self._server_config.get(
            'checkpoint_name', 'glofair_global_model.h5')
        ...
        self.server = ServerFactory().create('server_glofair',
                                             ...
                                             log_model=self._server_config.get(
                                                 'log_model', False),
