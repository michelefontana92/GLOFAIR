...
class RequirementSet:
    ...
    def evaluate(self, y_pred, y_true, group_ids):
        ...
        for requirement in self.requirements:
            ...
            results_dict.update(
                {requirement.name: eval_dict[requirement.name]*current_penalty})

        total_distance = round(
            sum([result for result in results_dict.values()]) / self.total_weight, 3)
