...
class WandbLogger(BaseLogger):
    def __init__(self, **kwargs):
        ...
        self.model_artifact = wandb.Artifact(
            name=self.checkpoint_path.split('.')[0], type='model')
    ...
    def serialize(self):
        ...
        return {
            ...
            'prefix': self.prefix,
            'include_context': self.include_context,
            # Presuppone che data_module abbia un metodo serialize
            'data_module': self.data_module.serialize() if self.data_module else None
        }
    ...
    def deserialize(data):
        ...
        logger = WandbLogger(
            ...
            data_module=DataModule.deserialize(
                data['data_module']) if data['data_module'] else None
        )
        return logger
    ...
    def close(self):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, self.checkpoint_path)):
            self.model_artifact.add_file(os.path.join(
